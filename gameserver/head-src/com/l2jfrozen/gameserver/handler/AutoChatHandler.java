...
public class AutoChatHandler implements SpawnListener
{
	...
	private void restoreChatData()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT * FROM auto_chat ORDER BY groupId ASC", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
			final ResultSet rs = statement.executeQuery();
			
			while (rs.next())
			{
				numLoaded++;
				PreparedStatement statement2 = con.prepareStatement("SELECT * FROM auto_chat_text WHERE groupId=?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
